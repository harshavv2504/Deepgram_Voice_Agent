...
class VoiceAgent:
    ...
    async def receiver(self):
        try:
            self.speaker = Speaker(browser_output=self.browser_audio)
...
class Speaker:
    ...
    def __enter__(self):
        # Always create queue and stop event
        self._queue = janus.Queue()
        self._stop = threading.Event()
        
        if not self.browser_output:
            # Server-side audio: initialize PyAudio
            ...
            # Frontend audio: create thread to process queue and send to browser
            self._thread = threading.Thread(
                target=_play_browser,
                args=(self._queue, self._stop, self.agent_audio_sample_rate),
                daemon=True,
            )
    ...
    async def play(self, data):
        # Always put audio data in queue for sequential processing
        if self._queue:
...
def _play(audio_out, stream, stop, browser_output=False):
    # This function is only used for server-side audio playback
    while not stop.is_set():
        try:
            data = audio_out.sync_q.get(True, 0.05)
            # Play audio through system speakers only
            stream.write(data)
        except queue.Empty:
            pass


def _play_browser(audio_out, stop, sample_rate):
    # This function processes audio queue for frontend audio
    while not stop.is_set():
        try:
            data = audio_out.sync_q.get(True, 0.05)
            # Send audio to browser via WebSocket with proper timing
            if socketio and data:
                try:
                    # Calculate proper delay based on audio chunk size and sample rate
                    # Assuming 16-bit audio (2 bytes per sample) and mono channel
                    bytes_per_sample = 2
                    samples_in_chunk = len(data) // bytes_per_sample
                    chunk_duration = samples_in_chunk / sample_rate
                    
                    # Ensure chunk_duration is reasonable (prevent extremely long delays)
                    if chunk_duration > 1.0:  # Cap at 1 second
                        chunk_duration = 0.1
                    elif chunk_duration < 0.001:  # Minimum 1ms
                        chunk_duration = 0.001
                    
                    # Send audio chunk to browser
                    socketio.emit(
                        "audio_output",
                        {"audio": data, "sampleRate": sample_rate},
                    )
                    
                    # Wait for the duration of this audio chunk before processing the next
            ...
            logger.error(f"Unexpected error in _play_browser: {e}")
            ...
            try:
                ...
                if not hasattr(handle_audio_data, "first_log_done"):
                    try:
                        ...
                        logger.info("NumPy not available, skipping sample analysis")
                    
                    logger.info(f"Received first browser audio chunk: {len(audio_bytes)} bytes, sample rate: {sample_rate}Hz")
                ...
                if voice_agent.loop and not voice_agent.loop.is_closed():
                    ...
                    logger.warning("Voice agent loop not available for audio processing")

            except Exception as e:
                logger.error(f"Error converting audio buffer: {e}, type: {type(audio_buffer)}")
                import traceback
                logger.error(traceback.format_exc())
